...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", help="the directory in which the data is stored", type=str, default='./datasets/RSNA_ASNR_MICCAI_BraTS2021_TrainingData_16July2021')
    parser.add_argument("--output_dir", help="the directory to store the preprocessed data", type=str, default='./data/BRATS')
    ...
    if not os.path.exists(img_output_root):
    ...
    img_output_root_val = os.path.join(img_output_root, 'val')
    train_txt = './brats_split_training.txt'
    test_txt = './brats_split_testing.txt'
    val_txt = './brats_split_validation.txt'
    ...
    with open(val_txt) as file:
        for path in file:
            nii2np_train(img_root, path[:-1], upper_per=args.upper_per, lower_per=args.lower_per, output_root_train=img_output_root_val, modality=MOD)